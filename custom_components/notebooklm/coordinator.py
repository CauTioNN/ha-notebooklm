...
class NotebookLMCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(
        ...
        # Snapshot of the entry options so the update listener can tell an
        # options change (reload) from a cookie/data write (ignore). Prevents a
        # reload loop driven by rotated cookies being saved back to the entry.
