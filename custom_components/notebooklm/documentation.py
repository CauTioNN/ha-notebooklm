...
from .const import (
    ...
    DOC_SCHEDULES,
    DOC_SOURCE_PREFIX,
    DOC_SYNC_STORAGE_KEY,
    DOC_SYNC_STORAGE_VERSION,
    DOC_SYNC_STORE_DATA,
    CONF_DOC_CATEGORIES,
    CONF_DOC_NOTEBOOK,
    CONF_DOC_SCRUB,
...
def _own_entity_ids(hass: HomeAssistant) -> set[str]:
    """Entity ids created by this integration itself.

    Excluded from the export so the documentation notebook never ingests the
    integration's own question/answer/status entities. Otherwise NotebookLM
    "sees" the stored ``text.notebooklm_question`` (and last answer) in its own
    sources and starts answering about itself — a feedback loop.
    """
...
def _build_overview(hass: HomeAssistant, deep: bool) -> str:
    cfg = hass.config
    own = _own_entity_ids(hass)
    states = [s for s in hass.states.async_all() if s.entity_id not in own]
...
def _build_areas(hass: HomeAssistant, deep: bool) -> str:
    ...
    for ent in ent_reg.entities.values():
        if ent.platform == DOMAIN:
            continue  # never document our own entities
...
# =============================================================================
# Persistent last-sync state
# =============================================================================
def _doc_sync_store(hass: HomeAssistant) -> Store:
    """Shared ``.storage/notebooklm_doc_sync`` store (one per HA instance).

    Keyed by notebook id rather than config-entry id, so the "last synced"
    timestamp survives an accidental delete + re-add of the integration: the
    file is not tied to any entry, and re-adding the same documentation
    notebook restores its date.
    """
    store: Store | None = hass.data.get(DOC_SYNC_STORE_DATA)
    if store is None:
        store = Store(hass, DOC_SYNC_STORAGE_VERSION, DOC_SYNC_STORAGE_KEY)
        hass.data[DOC_SYNC_STORE_DATA] = store
    return store
...
class DocSyncManager:
    ...
    async def async_load(self) -> None:
        """Restore the last-sync state for the configured notebook, if any.

        Called once during setup. Reads the standalone ``.storage`` file so the
        last-sync date shows up immediately after a restart — and after a
        delete + re-add, as long as the same documentation notebook is chosen.
        """
        notebook = self.configured_notebook()
        if not notebook:
            return
        data = await _doc_sync_store(self.hass).async_load() or {}
        record = (data.get("notebooks") or {}).get(notebook)
        if not record:
            return
        stamp = record.get("last_sync")
        if stamp:
            parsed = dt_util.parse_datetime(stamp)
            if parsed is not None:
                self.last_sync = parsed
        # Never restore a transient "running" status from a crashed run.
        status = record.get("last_status")
        if status in ("ok", "error"):
            self.last_status = status
        self.last_error = record.get("last_error")
        self.last_categories = record.get("last_categories") or self.last_categories
        self.last_source_count = record.get("last_source_count") or self.last_source_count

    async def _async_persist(self, notebook_id: str) -> None:
        """Write this notebook's last-sync state to the standalone store."""
        store = _doc_sync_store(self.hass)
        ...
        notebooks[notebook_id] = {
            ...
            "last_source_count": self.last_source_count,
        }
    ...
    async def _run(
        ...
        # Persist the date outside the config entry so it survives a restart
        # and an accidental delete + re-add of the integration.
        await self._async_persist(notebook_id)
        self.coordinator.async_update_listeners()
