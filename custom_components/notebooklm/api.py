...
async def async_validate_storage_state(
    ...
    Returns a dict with ``storage_state`` (parsed), ``account_email`` (or
    ``None``) and ``notebooks`` (a list of ``{"id", "title"}`` so the config
    flow can offer a required default-notebook picker). Raises
    :class:`InvalidStorageState` / :class:`AuthFailed` / :class:`CannotConnect`
    for the config flow to map to form errors.
    """
...
async def async_create_notebook_oneshot(
    hass: HomeAssistant, storage_state: Any, title: str
) -> dict[str, str]:
    """Create a notebook with a short-lived client (used during the config flow).

    Returns ``{"id", "title"}``. Lets exceptions propagate so the flow can map
    them to a ``create_failed`` form error.
    """
    await async_import_client(hass)
    from notebooklm import NotebookLMClient

    tmp = hass.config.path(STORAGE_SUBDIR, "notebooklm_validate.json")
    await hass.async_add_executor_job(_write_storage_state, tmp, storage_state)
    try:
        async with NotebookLMClient.from_storage(path=tmp) as client:
