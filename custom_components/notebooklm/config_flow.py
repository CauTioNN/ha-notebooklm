...
class NotebookLMConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    # Validated credentials + notebook list, carried from the auth step into the
    # mandatory default-notebook step.
    _validated: dict[str, Any] = {}
    ...
    async def _async_finish(self, validated: dict[str, Any]) -> ConfigFlowResult:
        """Update the entry on reauth, else go pick a required default notebook."""
        ...
        # First-time setup: require choosing the default notebook before the
        # entry is created, so every service/button has a target from the start.
        self._validated = validated
        return await self.async_step_notebook()

    async def async_step_notebook(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Mandatory step: pick (or create) the default notebook."""
        errors: dict[str, str] = {}
        notebooks: list[dict[str, str]] = self._validated.get("notebooks", [])

        if user_input is not None:
            notebook_id = user_input.get(CONF_DEFAULT_NOTEBOOK)
            new_title = (user_input.get("create_notebook") or "").strip()
            if new_title:
                try:
                    nb = await async_create_notebook_oneshot(
                        self.hass, self._validated["storage_state"], new_title
                    )
                    notebook_id = nb["id"]
                except Exception:  # noqa: BLE001 - surface as a form error
                    errors["base"] = "create_failed"
            if not errors and not notebook_id:
                errors["base"] = "notebook_required"
            if not errors:
                account_email = self._validated.get("account_email")
                return self.async_create_entry(
                    title=account_email or "NotebookLM",
                    data={
                        CONF_STORAGE_STATE: self._validated["storage_state"],
                        CONF_ACCOUNT_EMAIL: account_email,
                    },
                    options={CONF_DEFAULT_NOTEBOOK: notebook_id},
                )

        schema_dict: dict[Any, Any] = {}
        if notebooks:
            schema_dict[vol.Optional(CONF_DEFAULT_NOTEBOOK)] = SelectSelector(
                SelectSelectorConfig(
                    options=[
                        SelectOptionDict(value=nb["id"], label=nb["title"])
                        for nb in notebooks
                    ],
            ...
            if new_title:
                ...
                if coordinator is None:
                    ...
                    try:
                        ...
                        errors["base"] = "create_failed"
            # Documentation is opt-in, but if you open this step you must point it
            # at a notebook — pick an existing one or create one above.
            if not errors and not notebook_id:
                errors["base"] = "doc_notebook_required"
            if not errors:
                return self._save(
                    {
