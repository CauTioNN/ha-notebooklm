...
class NotebookLMActiveNotebookSelect(
    CoordinatorEntity[NotebookLMCoordinator], SelectEntity
):
    """Dropdown of the account's notebooks; selection drives service defaults.

    Intentionally **not** restored across restarts: every reload re-seeds the
    active notebook from the configured **default notebook** (see the coordinator)
    rather than re-applying a previous manual pick, so a restart returns to the
    default. A manual change still sticks until the next reload.
    """
